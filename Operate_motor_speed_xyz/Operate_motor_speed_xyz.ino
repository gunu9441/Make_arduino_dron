...
double eAngleZ = tAngleZ - AngleZ; //new12 rpy

double Kp = 1.0; // Kp =a:증폭값
//y = a*(-AngleY) 
double BalX = Kp * eAngleX;
double BalY = Kp * eAngleY;
double BalZ = Kp * eAngleZ;//new12 rpy

//new06 end 좌우균형값 찾기
//new11 start 회전속도로 좌우균형값 찾기
double Kd = 1.0;
BalX += Kd *(-GyXR); //y = b *(-w);
BalY += Kd *(-GyYR);
BalZ += Kd *(-GyZR); //new12 rpy
if (throttle ==0) BalX=BalY=BalZ = 0;//new 11 추가 //new12 rpy
//new11 end
//08 수정 static int throttle = 0;//100
//new08 start--> new10 serial --> serial1
